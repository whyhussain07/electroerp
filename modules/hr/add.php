...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    $pdo->prepare("INSERT INTO employees (name, phone, email, designation, department, salary, joining_date, address, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
        ->execute([trim($_POST['name']), trim($_POST['phone']), trim($_POST['email']), trim($_POST['designation']), trim($_POST['department']), (float)$_POST['salary'], $_POST['joining_date'], trim($_POST['address']), trim($_POST['notes'])]);
...
<head>
    <meta charset="UTF-8">
    <title>Add Employee | ElectroERP</title>
    <style>
...
<div class="content">
    ...
    <form method="POST">
        <div class="form-grid">
            <div class="form-group full"><label>Full Name *</label><input type="text" name="name" required placeholder="Employee name"></div>
            <div class="form-group"><label>Phone</label><input type="text" name="phone" placeholder="03001234567"></div>
            <div class="form-group"><label>Email</label><input type="email" name="email" placeholder="employee@email.com"></div>
            <div class="form-group"><label>Designation</label><input type="text" name="designation" placeholder="e.g. Salesman, Cashier, Manager"></div>
            <div class="form-group"><label>Department</label><input type="text" name="department" placeholder="e.g. Sales, Accounts"></div>
            <div class="form-group"><label>Monthly Salary (PKR)</label><input type="number" name="salary" value="0" step="0.01"></div>
            <div class="form-group"><label>Joining Date</label><input type="date" name="joining_date" value="<?php echo date('Y-m-d'); ?>"></div>
            <div class="form-group full"><label>Address</label><textarea name="address" placeholder="Home address"></textarea></div>
