...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $pdo->prepare("INSERT INTO customers (name, phone, email, address, city, balance, notes) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([trim($_POST['name']), trim($_POST['phone']), trim($_POST['email']), trim($_POST['address']), trim($_POST['city']), (float)$_POST['balance'], trim($_POST['notes'])]);
...
<head>
    <meta charset="UTF-8">
    <title>Add Customer | ElectroERP</title>
    <style>
...
<div class="content">
    ...
    <form method="POST">
        <div class="form-grid">
            <div class="form-group full"><label>Customer Name *</label><input type="text" name="name" required placeholder="e.g. Ahmed Ali"></div>
            ...
            <div class="form-group"><label>City</label><input type="text" name="city" placeholder="e.g. Gujrat"></div>
            <div class="form-group"><label>Opening Balance (PKR)</label><input type="number" name="balance" value="0" step="0.01"></div>
            <div class="form-group full"><label>Address</label><textarea name="address" placeholder="Full address"></textarea></div>
            <div class="form-group full"><label>Notes</label><textarea name="notes" placeholder="Any notes"></textarea></div>
        </div>
